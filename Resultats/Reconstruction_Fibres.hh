...
TH2F* Histo_Reconstruction_2D(const char* filename, const char* name)
{
  ...
  Tree->SetBranchAddress("DetectorPositionX", &vx);
  Tree->SetBranchAddress("DetectorPositionY", &vy);
...
TH2F* Histo_Reconstruction_Energie_Position(const char* filename, const char* name)
{
...
void Draw_ProjX_Test(TH2F* EPos, TH1D* ProjX,  TH1F* Estart, Int_t bin)
{
  ...
  cout << "Mean histo = " << ProjX->GetMean(1) << endl;
  cout << "RMS histo = " << ProjX->GetRMS(1) << endl;
...
void Draw_Results(TH1F* Estart, TH2F* Pos, TH2F* EPos, TGraphErrors* gResolution, TGraphErrors* Nph)
{
  ...
  Nph->Draw("APE2");
  Nph->SetTitle("");
  Nph->GetYaxis()->SetRangeUser(0.1, 100);
